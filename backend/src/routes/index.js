...
function createApiRouter(config) {
  ...
  function buildStorefrontSnippetJsManualBundles(merchantId, token) {
    ...
    parts.push(
      "async function tryApplyCoupon(code){var c=String(code||\"\").trim();if(!c)return false;function sleep(ms){return new Promise(function(r){setTimeout(r,ms)})}for(var attempt=0;attempt<3;attempt++){try{var cart=window.salla&&window.salla.cart;var applied=false;if(cart&&typeof cart.applyCoupon===\"function\"){await cart.applyCoupon(c);applied=true}else if(cart&&cart.coupon&&typeof cart.coupon.apply===\"function\"){await cart.coupon.apply(c);applied=true}else if(cart&&cart.coupon&&typeof cart.coupon.set===\"function\"){await cart.coupon.set(c);applied=true}else if(cart&&typeof cart.setCoupon===\"function\"){await cart.setCoupon(c);applied=true}else if(cart&&typeof cart.addCoupon===\"function\"){await cart.addCoupon(c);applied=true}else if(window.salla&&typeof window.salla.applyCoupon===\"function\"){await window.salla.applyCoupon(c);applied=true}else if(window.salla&&window.salla.coupon&&typeof window.salla.coupon.apply===\"function\"){await window.salla.coupon.apply(c);applied=true}if(applied){try{g.BundleApp._lastCouponApplyStatus=null;g.BundleApp._lastCouponApplyMessage=\"\"}catch(x0){}return true}}catch(e){var st=extractHttpStatus(e);var msg=extractHttpMessage(e);try{g.BundleApp._lastCouponApplyStatus=st;g.BundleApp._lastCouponApplyMessage=String(msg||\"\")}catch(x1){}var m=String(msg||\"\");if(st===410||m.indexOf('لم يعد متاحا')!==-1||m.toLowerCase().indexOf('no longer available')!==-1){return false}warn(\"bundle-app: coupon apply failed\",e&&((e.details)||e.message||e))}await sleep(250+attempt*250)}return false}"
    );
    parts.push(
