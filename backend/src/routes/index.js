...
function createApiRouter(config) {
  ...
  function buildStorefrontSnippetJsManualBundles(merchantId, token) {
    ...
    parts.push(
      "function markStoreClosed(e){try{var st=extractHttpStatus(e);var msg=extractHttpMessage(e);var m=String(msg||\"\");if(st===410||m.indexOf('المتجر مغلق')!==-1||m.indexOf('لم يعد متاحا')!==-1||m.toLowerCase().indexOf('no longer available')!==-1){g.BundleApp._storeClosedUntil=Date.now()+60000}}catch(x){}}"
    );
    parts.push(
