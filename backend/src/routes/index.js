...
function createApiRouter(config) {
  ...
  function buildStorefrontSnippetJsManualBundles(merchantId, token) {
    ...
    parts.push(
      "function findBannerMount(){try{var priceSelectors=['[data-testid=\"product-price\"]','.salla-product-price','.product-price','[class*=\"product-price\"]','[data-testid=\"product-price-value\"]','[data-price]','.price','[class*=\"price\"]'];for(var i=0;i<priceSelectors.length;i++){var el=document.querySelector(priceSelectors[i]);if(!el)continue;var host=el.closest('[data-testid=\"product-price\"],.salla-product-price,.product-price,[class*=\"product-price\"]')||el;var ins=host;try{var p=host&&host.parentNode;if(p&&p.nodeType===1&&typeof window.getComputedStyle===\"function\"){var cs=window.getComputedStyle(p);var d=cs&&cs.display;var fd=cs&&cs.flexDirection;if(d===\"flex\"&&fd&&fd.indexOf(\"row\")!==-1){ins=p}}}catch(x0){}return{el:ins,where:\"after\"}}var btnSelectors=['[data-testid=\"add-to-cart\"]','button[name=\"add-to-cart\"]','button[type=\"submit\"]','.salla-add-to-cart-button','.add-to-cart'];for(var j=0;j<btnSelectors.length;j++){var b=document.querySelector(btnSelectors[j]);if(b)return{el:b,where:\"before\"}}return null}catch(e){return null}}"
    );
    parts.push(
      "function mountBanner(root){try{var m=findBannerMount();if(m&&m.el&&m.el.parentNode){root.className=\"bundle-app-container bundle-app-banner--inline\";try{root.style.display=\"block\";root.style.width=\"100%\"}catch(x0){}if(m.where===\"before\"){m.el.parentNode.insertBefore(root,m.el)}else{m.el.parentNode.insertBefore(root,m.el.nextSibling)}return true}}catch(e){}root.className=\"bundle-app-container bundle-app-banner--fixed\";if(!root.parentNode||root.parentNode!==document.body)document.body.appendChild(root);return false}"
    );
    parts.push(
      "function escHtml(input){var s=String(input==null?\"\":input);return s.replace(/[&<>\"']/g,function(ch){return ch===\"&\"?\"&amp;\":ch===\"<\"?\"&lt;\":ch===\">\"?\"&gt;\":ch==='\"'?\"&quot;\":\"&#39;\"})}"
    );
