...
function createApiRouter(config) {
  ...
  function buildStorefrontSnippetJsV2(merchantId, token) {
    ...
    parts.push(
      "function buildItemsText(bundle){var items=(bundle&&bundle.bundleItems)||[];if(!Array.isArray(items)||!items.length)return\"\";var totalQty=0;for(var i=0;i<items.length;i++){totalQty+=Math.max(1,Math.floor(Number(items[i]&&items[i].quantity||1)))}return \"عدد المنتجات: \"+fmtNum(items.length)+\" • إجمالي القطع: \"+fmtNum(totalQty)}"
    );
    parts.push(
      "function buildTierRows(bundle){var p=bundle&&bundle.pricing;var offer=bundle&&bundle.offer||{};var rows=[];var tiers=p&&p.tiers;if(Array.isArray(tiers)&&tiers.length){for(var i=0;i<tiers.length;i++){var t=tiers[i]||{};rows.push({minQty:t.minQty,type:t.type,value:t.value,originalTotal:t.originalTotal,finalTotal:t.finalTotal,discountAmount:t.discountAmount})}}else if(p&&p.base){rows.push({minQty:1,type:offer.type,value:offer.value,originalTotal:p.base.originalTotal,finalTotal:p.base.finalTotal,discountAmount:p.base.discountAmount})}rows=rows.filter(function(r){return r&&Number(r.minQty)>=1});rows.sort(function(a,b){return a.minQty-b.minQty});var out='';for(var j=0;j<rows.length;j++){var r=rows[j];var pct=pctFrom(r.originalTotal,r.finalTotal);var left='عند '+fmtNum(r.minQty)+' قطع';var right='قبل '+fmtMoney(r.originalTotal)+' • بعد '+fmtMoney(r.finalTotal);var save=Number(r.discountAmount);if(Number.isFinite(save)&&save>0){right+=' • وفّرت '+fmtMoney(save);if(pct!=null)right+=' ('+fmtNum(pct)+'%)'}out+='<div class=\"bundle-app-tier\"><div><strong>'+escHtml(left)+'</strong></div><div class=\"bundle-app-muted\">'+escHtml(right)+'</div></div>'}return out}"
    );
    parts.push(
      "function renderProductBanners(bundles){ensureStyles();var id=\"bundle-app-banner\";var root=document.getElementById(id);if(!root){root=document.createElement(\"div\");root.id=id}mountBanner(root);var arr=Array.isArray(bundles)?bundles:[];if(!arr.length){clearProductBanner();return}var html='';for(var i=0;i<arr.length;i++){var b=arr[i]||{};var color=String(b.bannerColor||\"#0ea5e9\");var title=normalizeTitle(b.title);var itemsText=buildItemsText(b);var tiersHtml=buildTierRows(b);var cta=String(b.cta||\"أضف الباقة\");html+='<div class=\"bundle-app-card\" style=\"background:'+escHtml(color)+'\"><div class=\"bundle-app-row\"><div><div class=\"bundle-app-title\">'+escHtml(title)+'</div>'+(itemsText?('<div class=\"bundle-app-items\">'+escHtml(itemsText)+'</div>'):'')+'</div><button class=\"bundle-app-btn\" type=\"button\" data-bundle-index=\"'+i+'\">'+escHtml(cta)+'</button></div>'+(tiersHtml?('<div class=\"bundle-app-tiers\">'+tiersHtml+'</div>'):'')+'</div>'}root.innerHTML=html;var btns=root.querySelectorAll('[data-bundle-index]');for(var k=0;k<btns.length;k++){(function(btn){var idx=Number(btn.getAttribute('data-bundle-index'));btn.onclick=async function(){try{btn.disabled=true;var bundle=arr[idx]||{};var items=(bundle&&bundle.bundleItems)||[];for(var ii=0;ii<items.length;ii++){var it=items[ii]||{};var pid=Number(it.productId);var qty=Math.max(1,Math.floor(Number(it.quantity||1)));if(!Number.isFinite(pid)||pid<=0)continue;if(window.salla&&window.salla.cart&&typeof window.salla.cart.addItem===\"function\"){await window.salla.cart.addItem({id:pid,quantity:qty})}}btn.disabled=false}catch(e){btn.disabled=false;warn(\"bundle-app: add-to-cart failed\",e&&((e.details)||e.message||e))}};})(btns[k])} }"
    );
    ...
    parts.push(
      "g.BundleApp.renderTest=function(){renderProductBanners([{bannerColor:\"#0ea5e9\",title:\"باقة تجريبية\",cta:\"أضف الباقة\",bundleItems:[{productId:\"154200631\",quantity:1}],offer:{type:\"percentage\",value:10,tiers:[{minQty:2,type:\"percentage\",value:10},{minQty:4,type:\"percentage\",value:30}]},pricing:{base:{originalTotal:3000,discountAmount:300,finalTotal:2700},tiers:[{minQty:2,type:\"percentage\",value:10,originalTotal:6000,discountAmount:600,finalTotal:5400},{minQty:4,type:\"percentage\",value:30,originalTotal:12000,discountAmount:3600,finalTotal:8400}]}}])};"
    );
    parts.push("initAuto();");
    parts.push("})();");
    return parts.join("");
