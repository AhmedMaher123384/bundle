...
function createApiRouter(config) {
  ...
  function buildStorefrontSnippetJsManualBundles(merchantId, token) {
    ...
    parts.push(
      "function ensurePickerStyles(){if(document.getElementById(\"bundle-app-pickers-style\"))return;var s=document.createElement(\"style\");s.id=\"bundle-app-pickers-style\";s.textContent='.bundle-app-pickers{margin-top:10px;display:none;background:rgba(255,255,255,.14);border-radius:12px;padding:10px}.bundle-app-card:hover .bundle-app-pickers,.bundle-app-card--selected .bundle-app-pickers{display:block}.bundle-app-pickers-title{font-weight:900;font-size:12px;margin-bottom:8px}.bundle-app-picker-row{display:flex;flex-direction:column;gap:6px;margin-bottom:8px}.bundle-app-picker-label{font-weight:800;font-size:12px}.bundle-app-picker-select{width:100%;padding:10px;border-radius:10px;border:1px solid rgba(255,255,255,.55);background:rgba(255,255,255,.95);color:#111827}.bundle-app-picker-hint{font-size:12px;opacity:.95}.bundle-app-picker-status{font-size:12px;opacity:.95;margin-top:6px}';document.head.appendChild(s)}"
    );
    parts.push(
    ...
    parts.push(
      "function applySelectionsToContainer(container,bundleId){try{if(!container)return;var bid=String(bundleId||\"\").trim();if(!bid)return;var sel=getBundleVariantSelectionMap(bid)||{};var selects=container.querySelectorAll('select[data-unit-key]');for(var i=0;i<selects.length;i++){var s=selects[i];var key=String(s.getAttribute('data-unit-key')||\"\");if(!key)continue;var val=String(sel[key]||\"\").trim();if(val)s.value=val}}catch(e){}}"
    );
    parts.push(
      "function updatePickerStatus(container){try{if(!container)return;var selects=container.querySelectorAll('select[data-unit-key]');var total=selects.length;var chosen=0;for(var i=0;i<selects.length;i++){var v=String(selects[i].value||\"\").trim();if(v)chosen++}var el=container.querySelector('[data-role=\"picker-status\"]');if(el)el.textContent=total?('تم اختيار '+fmtNum(chosen)+' من '+fmtNum(total)):'')}catch(e){}}"
    );
    parts.push(
      "function bindPickerContainer(container,bundleId){try{if(!container)return;var bid=String(bundleId||\"\").trim();if(!bid)return;container.onchange=function(e){var t=e&&e.target;if(!t||t.tagName!==\"SELECT\")return;var key=String(t.getAttribute('data-unit-key')||\"\").trim();if(!key)return;var val=String(t.value||\"\").trim();var sel=getBundleVariantSelectionMap(bid);if(!sel)return;if(val){sel[key]=val}else{try{delete sel[key]}catch(x){sel[key]=null}}updatePickerStatus(container)};applySelectionsToContainer(container,bid);updatePickerStatus(container)}catch(e){}}"
    );
    parts.push(
      "async function ensureVariantPickersForCard(card,bundle){try{if(!card||!bundle)return;ensurePickerStyles();var bid=String(card.getAttribute('data-bundle-id')||\"\").trim();if(!bid)return;var container=card.querySelector('.bundle-app-pickers[data-bundle-id]');if(!container)return;var sig=bundleVariantSig(bundle);var cached=variantPickerCacheByBundleId[bid];if(cached&&cached.sig===sig&&cached.html!=null){container.innerHTML=cached.html;bindPickerContainer(container,bid);return}var pending=variantPickerPendingByBundleId[bid];if(pending&&pending.sig===sig&&pending.promise)return;container.innerHTML='<div class=\"bundle-app-picker-hint\">جاري تحميل الفاريانت...</div>';var promise=(async function(){var units=bundleVariantUnits(bundle);var sel=getBundleVariantSelectionMap(bid);if(!units.length){variantPickerCacheByBundleId[bid]={sig:sig,html:\"\"};container.innerHTML='';return}pruneBundleSelections(sel,units);var varsByPid={};var uniq={};for(var i=0;i<units.length;i++){uniq[String(units[i].productId)]=true}var pids=Object.keys(uniq);for(var j=0;j<pids.length;j++){var pid=pids[j];var vr=await getCachedVariants(pid);varsByPid[pid]=Array.isArray(vr)?vr:[]}var html='<div class=\"bundle-app-pickers-title\">اختيار الفاريانت للكميات</div><div class=\"bundle-app-picker-status\" data-role=\"picker-status\"></div>';for(var k=0;k<units.length;k++){var unit=units[k];var list=(varsByPid[unit.productId]||[]).filter(function(x){return x&&x.isActive===true&&String(x.variantId||\"\").trim()});html+='<div class=\"bundle-app-picker-row\"><div class=\"bundle-app-picker-label\">قطعة '+fmtNum(k+1)+'</div>';if(list.length===1){var only=list[0]||{};var vid=String(only.variantId||\"\").trim();if(vid){sel[unit.key]=vid}html+='<select class=\"bundle-app-picker-select\" data-unit-key=\"'+escHtml(unit.key)+'\" disabled><option value=\"'+escHtml(vid)+'\">'+escHtml(variantLabel(only))+'</option></select>'}else{html+='<select class=\"bundle-app-picker-select\" data-unit-key=\"'+escHtml(unit.key)+'\"><option value=\"\">— اختر —</option>';for(var o=0;o<list.length;o++){var vv=list[o]||{};var ov=String(vv.variantId||\"\").trim();if(!ov)continue;html+='<option value=\"'+escHtml(ov)+'\">'+escHtml(variantLabel(vv))+'</option>'}html+='</select>'}html+='</div>'}variantPickerCacheByBundleId[bid]={sig:sig,html:html};container.innerHTML=html;bindPickerContainer(container,bid)})();variantPickerPendingByBundleId[bid]={sig:sig,promise:promise};await promise;variantPickerPendingByBundleId[bid]=null}catch(e){}}"
    );
    parts.push(
