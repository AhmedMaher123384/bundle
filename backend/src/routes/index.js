...
// ✅ ✅ ✅ START OF MODIFIED FRONTEND SNIPPET — WHITE, ELEGANT, FAST, RESPONSIVE ✅ ✅ ✅
...
parts.push("var lastRenderedBundles=null;");
parts.push("var idleCallbackId=null;");
parts.push("var isPageVisible=true;");
parts.push("var isIntersecting=false;");

// ✅ ENSURE STYLES — COMPLETELY REBUILT FOR ELEGANCE, WHITE THEME, BURGUNDY ACCENT
parts.push(
`function ensureStyles(){
if(document.getElementById("bundle-app-style"))return;
var s=document.createElement("style");s.id="bundle-app-style";
s.textContent=\`
:root {
  --bundle-bg: #ffffff;
  --bundle-border: #f3f4f6;
  --bundle-shadow: 0 4px 20px rgba(0,0,0,0.06);
  --bundle-radius: 16px;
  --bundle-gap: 16px;
  --bundle-color-text: #1F2937;
  --bundle-color-muted: #6B7280;
  --bundle-color-price: #8B0026; /* Burgundy per your preference */
  --bundle-color-cta: #8B0026;
  --bundle-color-cta-hover: #7A0020;
  --bundle-color-success: #059669;
  --bundle-color-error: #DC2626;
  --bundle-font: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif;
}
...
.bundle-app-container {
  font-family: var(--bundle-font);
  font-feature-settings: "rlig" 1, "calt" 1;
}

.bundle-app-banner--inline {
  position: relative;
  display: block;
  width: 100%;
  margin: 16px 0;
...
.bundle-app-card:hover {
  transform: translateY(-2px);
  box-shadow: 0 8px 24px rgba(0,0,0,0.08);
}

.bundle-app-card--selected {
  outline: 2px solid var(--bundle-color-cta);
  outline-offset: -2px;
}

.bundle-app-card+.bundle-app-card {
  margin-top: var(--bundle-gap);
}
...
.bundle-app-radio {
  margin-top: 6px;
  transform: scale(1.1);
  accent-color: var(--bundle-color-cta);
}
...
.bundle-app-btn {
  ...
  font-weight: 700;
  cursor: pointer;
  background: var(--bundle-color-cta);
  color: #fff;
  min-width: 110px;
...
.bundle-app-btn:hover:not(:disabled) {
  background: var(--bundle-color-cta-hover);
  transform: translateY(-1px);
}

.bundle-app-btn:active:not(:disabled) {
  transform: translateY(0);
}

.bundle-app-btn:disabled {
  opacity: 0.8;
  cursor: not-allowed;
  transform: none;
}
...
.bundle-app-msg--success {
  color: var(--bundle-color-success);
  background: #ecfdf5;
}

.bundle-app-msg--error {
  color: var(--bundle-color-error);
  background: #fef2f2;
}

/* Tiers — elegant, modern */
.bundle-app-tiers {
  margin-top: var(--bundle-gap);
  display: flex;
...
.bundle-app-tier:hover {
  background: #f5f5f5;
  border-color: #e5e5e5;
}

.bundle-app-tier--selected {
  background: #fff9f9;
  border-color: var(--bundle-color-cta);
  outline: 1px solid var(--bundle-color-cta);
  outline-offset: -1px;
}

.bundle-app-tier strong {
  font-weight: 700;
  color: var(--bundle-color-text);
}

.bundle-app-tier .bundle-app-tier-discount {
  color: var(--bundle-color-success);
  font-weight: 600;
}

/* Items Grid */
.bundle-app-items-grid {
  display: flex;
  gap: 8px;
  margin-top: 10px;
  flex-wrap: wrap;
...
.bundle-app-item-img img.loaded {
  opacity: 1;
}
...
/* Spinner */
@keyframes spin-fade {
  0% { opacity: 0.2; transform: rotate(0deg) scale(0.8); }
  50% { opacity: 1; }
  100% { opacity: 0.2; transform: rotate(360deg) scale(1.2); }
}
.bundle-app-spinner {
  ...
  border-top-color: var(--bundle-color-cta);
  border-radius: 50%;
...
/* RTL Support */
[dir="rtl"] .bundle-app-row,
[dir="rtl"] .bundle-app-choice,
[dir="rtl"] .bundle-app-items-grid {
  flex-direction: row-reverse;
}
[dir="rtl"] .bundle-app-radio {
  margin-right: 6px;
  margin-left: 0;
}
[dir="rtl"] .bundle-app-price {
  direction: rtl;
  text-align: end;
}
\`;
...
parts.push(
"function buildItemsGrid(items){if(!Array.isArray(items)||!items.length)return'';var html='<div class=\"bundle-app-items-grid\">';for(var i=0;i<items.length;i++){var it=items[i]||{};var imgSrc=it.imageUrl?escHtml(it.imageUrl):'';var fallbackBg='linear-gradient(135deg, #f9fafb, #f3f4f6)';var qty=fmtNum(it.quantity||1);html+='<div class=\"bundle-app-item\">'+(imgSrc?'<div class=\"bundle-app-item-img\"><img src=\"'+imgSrc+'\" loading=\"lazy\" alt=\"\" onerror=\"this.style.display=\\\'none\\\'\" onload=\"this.classList.add(\\\'loaded\\\')\" /></div>':'<div class=\"bundle-app-item-img\" style=\"background:'+fallbackBg+'\"></div>')+'<div class=\"bundle-app-item-qty\">'+escHtml(qty)+'</div></div>'}html+='</div>';return html}"
);
parts.push(
"function buildPriceText(bundle){var p=bundle&&bundle.pricing&&bundle.pricing.base;if(!p)return\"\";var o=Number(p.originalTotal),f=Number(p.finalTotal),d=Number(p.discountAmount);if(!Number.isFinite(o)||!Number.isFinite(f))return\"\";var s=fmtMoney(f);if(Number.isFinite(d)&&d>0)s+='<span style=\"color:#059669;margin-right:4px;font-weight:600\">- '+fmtMoney(d)+'</span>';return s}"
);
parts.push(
...
parts.push(
"async function tryClearCoupon(){/* optimized: keep but silent */try{var cart=window.salla&&window.salla.cart;if(!cart)return false;if(cart.coupon&&typeof cart.coupon.remove===\"function\"){await cart.coupon.remove();return true}if(typeof cart.removeCoupon===\"function\"){await cart.removeCoupon();return true}}catch(e){}return false}"
);
parts.push(
...
parts.push(
"async function applyPendingCouponForCart(){if(!isCartLikePage()||!isPageVisible)return;try{var trigger=String(lastTriggerProductId||\"\");if(!trigger)return;var pending=loadPendingCoupon(trigger);if(!pending||!pending.code)return;var ts=Number(pending.ts||0);if(!Number.isFinite(ts)||ts<=0||Date.now()-ts>10*60*1000){clearPendingCoupon(trigger);return}var ok=await tryApplyCoupon(pending.code);if(ok){clearPendingCoupon(trigger)}}catch(e){}}"
);

// ✅ REBUILT renderProductBanners — with price on top, elegant spacing, image grid
parts.push(
`function renderProductBanners(bundles){
ensureStyles();
var id="bundle-app-banner";
var root=document.getElementById(id);
if(!root){root=document.createElement("div");root.id=id}
mountBanner(root);

var arr=Array.isArray(bundles)?bundles:[];
if(!arr.length){clearProductBanner();return}

// Detect RTL
var isRTL=document.documentElement.getAttribute("dir")==="rtl"||document.body.getAttribute("dir")==="rtl"||window.getComputedStyle(document.body).direction==="rtl";
if(isRTL)root.setAttribute("dir","rtl");else root.removeAttribute("dir");

var trigger=String(arr[0]&&arr[0].triggerProductId||"");
lastTriggerProductId=trigger||lastTriggerProductId;

if(!selectedBundleId&&arr[0]&&arr[0].id)selectedBundleId=String(arr[0].id||"");

var html='';
for(var i=0;i<arr.length;i++){
  var b=arr[i]||{};
  var bid=String(b.id||"");
  var title=normalizeTitle(b.title);
  var items=normalizeItems(b);
  var itemsGrid=buildItemsGrid(items);
  var priceHtml=buildPriceText(b);
  var msg=String(messageByBundleId[bid]||"");
  var msgClass=msg?"bundle-app-msg":(b.hasDiscount?"bundle-app-msg bundle-app-msg--success":"");
  if(msg&&msg.indexOf('خطأ')!==-1||msg.indexOf('failed')!==-1)msgClass="bundle-app-msg bundle-app-msg--error";
  var checked=bid===String(selectedBundleId||"");

  html+='<div class="bundle-app-card'+(checked?' bundle-app-card--selected':'')+'" data-bundle-id="'+escHtml(bid)+'">';
  
  // ✅ PRICE ON TOP — ELEGANT, BURGUNDY, BOLD
  if(priceHtml){
    html+='<div class="bundle-app-price-wrap"><div class="bundle-app-price">'+priceHtml+'</div></div>';
  }
  
  html+='<div class="bundle-app-row">';
  html+='<label class="bundle-app-choice">';
  html+='<input class="bundle-app-radio" type="radio" name="bundle_app_choice" value="'+escHtml(bid)+'" '+(checked?'checked':'')+' />';
  ...
  if(msg){
    html+='<div class="'+msgClass+'">'+escHtml(msg)+(applying&&msg.indexOf('جاري')===0?' <span class="bundle-app-spinner"></span>':'')+'</div>';
  }
  
  html+='</div></label>';
  
  var btnLabel='أضف الباقة';
  var tiersHtml=buildTierRows(b,bid,pickMinQty(b));
  if(tiersHtml){
    btnLabel+=' ('+fmtNum(Math.max(getPageQty(),Math.max(minRequiredBaseQty(b),pickMinQty(b))))+' قطع)';
  }
  
  html+='<button class="bundle-app-btn" type="button" data-action="apply-one" data-bundle-id="'+escHtml(bid)+'" '+(applying?'disabled':'')+'>'+escHtml(btnLabel)+'</button>';
  html+='</div>'; // .bundle-app-row
  
  if(tiersHtml){
    html+='<div class="bundle-app-tiers">'+tiersHtml+'</div>';
  }
  
  html+='</div>'; // .bundle-app-card
}
...
root.onclick=function(e){
  ...
  // Radio selection
  if(target.matches('input.bundle-app-radio')){
    selectedBundleId=String(target.value||"");
    renderProductBanners(arr);
    return;
  }
  
  // Tier selection
  if(target.closest('.bundle-app-tier')){
    var el=target.closest('.bundle-app-tier');
    var bid=String(el.getAttribute('data-bundle-id')||"");
    var mq=Number(el.getAttribute('data-tier-minqty'));
  ...
  // Apply button
  if(target.matches('button.bundle-app-btn[data-action="apply-one"]')){
    if(applying)return;
    var btn=target;
    ...
    for(var j=0;j<arr.length;j++){
      if(String(arr[j]&&arr[j].id||"")===bid){chosen=arr[j];break}
    }
    if(!chosen)return;
    applyBundleSelection(chosen);
  }
};
}`
);

// ✅ Tiers — modern, with discount highlight
parts.push(
...
try{
  ...
  for(var i=0;i<tiers.length;i++){
    ...
    for(var j=0;j<pricingTiers.length;j++){
      ...
      if(Math.floor(Number(pt.minQty||0))===mq){pr=pt;break}
    }
    rows.push({minQty:mq,pricing:pr})
  }
  rows.sort(function(a,b){return a.minQty-b.minQty});
  var out='';
  for(var k=0;k<rows.length;k++){
    var r=rows[k];
    var prc=r.pricing||{};
    var o=Number(prc.originalTotal),f=Number(prc.finalTotal),d=Number(prc.discountAmount);
    var left='عند '+fmtNum(r.minQty)+' قطع';
    var right='';
    var discountHtml='';
    if(Number.isFinite(o)&&Number.isFinite(f)){
      right='بعد '+fmtMoney(f);
      if(Number.isFinite(d)&&d>0){
        discountHtml='<span class="bundle-app-tier-discount">• وفّرت '+fmtMoney(d)+'</span>';
      }
    }
    var cls='bundle-app-tier'+(Number(selectedMinQty)===Number(r.minQty)?' bundle-app-tier--selected':'');
    out+='<div class="'+cls+'" data-bundle-id="'+escHtml(bundleId)+'" data-tier-minqty="'+escHtml(r.minQty)+'"><div><strong>'+escHtml(left)+'</strong> '+discountHtml+'</div><div class="bundle-app-muted">'+escHtml(right)+'</div></div>'
  }
  return out;
}catch(e){return""}
}`
...
// ✅ PERFORMANCE: Replace setInterval with requestIdleCallback + visibility-aware refresh
parts.push(
`function shouldRefresh(){
  return isPageVisible && (isIntersecting || document.getElementById("bundle-app-banner"));
}
...
function initAuto(){
  ...
  function start(){
    ...
    // Observe banner visibility for performance
    try{
      var root=document.getElementById("bundle-app-banner")||document.createElement("div");
      root.id="bundle-app-banner";
      new IntersectionObserver(function(entries){
        isIntersecting=entries[0].isIntersecting;
        if(isIntersecting)scheduleRefresh();
    ...
    try{
      document.addEventListener("visibilitychange",function(){
        ...
        if(isPageVisible)scheduleRefresh();
      });
      window.addEventListener("focus",function(){isPageVisible=true;scheduleRefresh();});
    }catch(e){}
...
// ✅ ✅ ✅ END OF MODIFIED FRONTEND SNIPPET ✅ ✅ ✅
...
let js = buildStorefrontSnippetJsManualBundles(merchantId, token);
return res.send(js);
} catch (err) {
return next(err);
}
