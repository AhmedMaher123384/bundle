...
function createApiRouter(config) {
  ...
  function buildStorefrontSnippetJsV2(merchantId, token) {
    ...
    parts.push(
      "function renderProductBanners(bundles){ensureStyles();var id=\"bundle-app-banner\";var root=document.getElementById(id);if(!root){root=document.createElement(\"div\");root.id=id}mountBanner(root);var arr=Array.isArray(bundles)?bundles:[];if(!arr.length){clearProductBanner();return}var html='';for(var i=0;i<arr.length;i++){var b=arr[i]||{};var color=String(b.bannerColor||\"#0ea5e9\");var title=normalizeTitle(b.title);if(title.indexOf('وفر ')!==-1&&title.indexOf('وفر حتى ')===-1){try{title=title.replace('وفر ','وفر حتى ')}catch(e){}}var selectedQty=pickMinQty(b,i);var itemsText=buildItemsText(b,selectedQty);var tiersHtml=buildTierRows(b,i,selectedQty);var msg=String(messageByBundle[i]||\"\");var cta=String(b.cta||\"أضف الباقة\");var btnLabel=cta;var hasTiers=Boolean(b&&b.pricing&&Array.isArray(b.pricing.tiers)&&b.pricing.tiers.length);if(hasTiers){btnLabel=cta+' ('+fmtNum(selectedQty)+' قطع)'}html+='<div class=\"bundle-app-card\" style=\"background:'+escHtml(color)+'\"><div class=\"bundle-app-row\"><div><div class=\"bundle-app-title\">'+escHtml(title)+'</div>'+(itemsText?('<div class=\"bundle-app-items\">'+escHtml(itemsText)+'</div>'):'')+(msg?('<div class=\"bundle-app-msg\">'+escHtml(msg)+'</div>'):'')+'</div><button class=\"bundle-app-btn\" type=\"button\" data-bundle-index=\"'+i+'\">'+escHtml(btnLabel)+'</button></div>'+(tiersHtml?('<div class=\"bundle-app-tiers\">'+tiersHtml+'</div>'):'')+'</div>'}root.innerHTML=html;var tierEls=root.querySelectorAll('[data-tier-minqty][data-bundle-index]');for(var t=0;t<tierEls.length;t++){(function(el){el.onclick=function(){var bi=Number(el.getAttribute('data-bundle-index'));var mq=Number(el.getAttribute('data-tier-minqty'));if(Number.isFinite(bi)&&Number.isFinite(mq)&&mq>=1){selectedTierByBundle[bi]=Math.floor(mq);messageByBundle[bi]='';renderProductBanners(arr)}}})(tierEls[t])}var btns=root.querySelectorAll('[data-bundle-index]');for(var k=0;k<btns.length;k++){(function(btn){var idx=Number(btn.getAttribute('data-bundle-index'));btn.onclick=async function(){try{var bundle=arr[idx]||{};var selectedQty=pickMinQty(bundle,idx);messageByBundle[idx]='جاري إضافة الباقة...';renderProductBanners(arr);btn.disabled=true;var cartItems=buildCartItems(bundle,selectedQty);await addItemsToCart(cartItems);messageByBundle[idx]='جاري تطبيق الخصم...';renderProductBanners(arr);var banner=await applyDiscountForCartItems(cartItems);var code=banner&&banner.couponCode;var has=Boolean(banner&&banner.hasDiscount&&code);if(has){var applied=await tryApplyCoupon(code);if(applied){messageByBundle[idx]='تم تطبيق خصم الباقة ✓'}else{savePendingCoupon(code);messageByBundle[idx]='تم إنشاء كوبون الخصم وسيُطبّق في السلة: '+code}}else{messageByBundle[idx]='تمت إضافة الباقة. لم ينطبق خصم على هذه السلة.'}btn.disabled=false;renderProductBanners(arr)}catch(e){btn.disabled=false;messageByBundle[idx]='حصل خطأ أثناء الإضافة أو تطبيق الخصم';warn(\"bundle-app: add/apply failed\",e&&((e.details)||e.message||e));renderProductBanners(arr)}}})(btns[k])}}"
    );
