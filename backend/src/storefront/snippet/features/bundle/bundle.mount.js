...
module.exports = function mountBundle(context) {
  ...
  parts.push(
    'function initAuto(){var inited=false;var lastKey="";var lastEvt=0;function key(){try{var v=findVariantId();if(v)return"v:"+String(v);var p=findProductId();return p?"p:"+String(p):""}catch(e){return""}}function refreshIfChanged(){try{var k=key();if(!k){lastKey="";try{clearProductBanner()}catch(e0){}return}if(k===lastKey)return;lastKey=k;refreshProduct()}catch(e){}}function popupTick(){try{if(typeof popupRouteChanged==="function")popupRouteChanged()}catch(e){}}function onEvt(){var now=Date.now();if(now-lastEvt<350)return;lastEvt=now;setTimeout(function(){refreshIfChanged()},0);setTimeout(function(){popupTick()},0)}function start(){if(inited)return;inited=true;popupTick();refreshIfChanged();try{window.addEventListener("focus",onEvt);window.addEventListener("popstate",onEvt);window.addEventListener("hashchange",onEvt);document.addEventListener("visibilitychange",function(){if(document.visibilityState==="visible")onEvt()});document.addEventListener("change",onEvt,true);var h=window.history;if(h&&h.pushState){var p=h.pushState;h.pushState=function(){var r=p.apply(this,arguments);onEvt();return r}}if(h&&h.replaceState){var r0=h.replaceState;h.replaceState=function(){var r=r0.apply(this,arguments);onEvt();return r}}}catch(e){}setInterval(function(){refreshIfChanged();popupTick()},15000)}if(document.readyState==="loading"){document.addEventListener("DOMContentLoaded",start)}else{start()}}'
  );
