...
function createApiRouter(config) {
  ...
  function buildStorefrontSnippetJsV2(merchantId, token) {
    ...
    parts.push(
      "async function tryApplyCoupon(code){var c=String(code||\"\").trim();if(!c)return false;function sleep(ms){return new Promise(function(r){setTimeout(r,ms)})}for(var attempt=0;attempt<6;attempt++){try{var cart=window.salla&&window.salla.cart;var applied=false;var payload={code:c};if(cart&&typeof cart.applyCoupon===\"function\"){try{await cart.applyCoupon(c)}catch(e0){await cart.applyCoupon(payload)}applied=true}else if(cart&&cart.coupon&&typeof cart.coupon.apply===\"function\"){try{await cart.coupon.apply(c)}catch(e1){await cart.coupon.apply(payload)}applied=true}else if(cart&&cart.coupon&&typeof cart.coupon.set===\"function\"){try{await cart.coupon.set(c)}catch(e2){await cart.coupon.set(payload)}applied=true}else if(cart&&typeof cart.setCoupon===\"function\"){try{await cart.setCoupon(c)}catch(e3){await cart.setCoupon(payload)}applied=true}else if(cart&&typeof cart.addCoupon===\"function\"){try{await cart.addCoupon(c)}catch(e4){await cart.addCoupon(payload)}applied=true}else if(window.salla&&typeof window.salla.applyCoupon===\"function\"){try{await window.salla.applyCoupon(c)}catch(e5){await window.salla.applyCoupon(payload)}applied=true}else if(window.salla&&window.salla.coupon&&typeof window.salla.coupon.apply===\"function\"){try{await window.salla.coupon.apply(c)}catch(e6){await window.salla.coupon.apply(payload)}applied=true}if(applied)return true}catch(e){warn(\"bundle-app: coupon apply failed\",e&&((e.details)||e.message||e))}await sleep(400+attempt*250)}return false}"
    );
    parts.push(
  ...
  function buildStorefrontSnippetJsManualBundles(merchantId, token) {
    ...
    parts.push(
      "async function tryApplyCoupon(code){var c=String(code||\"\").trim();if(!c)return false;function sleep(ms){return new Promise(function(r){setTimeout(r,ms)})}for(var attempt=0;attempt<8;attempt++){try{var cart=window.salla&&window.salla.cart;var applied=false;var payload={code:c};if(cart&&typeof cart.applyCoupon===\"function\"){try{await cart.applyCoupon(c)}catch(e0){await cart.applyCoupon(payload)}applied=true}else if(cart&&cart.coupon&&typeof cart.coupon.apply===\"function\"){try{await cart.coupon.apply(c)}catch(e1){await cart.coupon.apply(payload)}applied=true}else if(cart&&cart.coupon&&typeof cart.coupon.set===\"function\"){try{await cart.coupon.set(c)}catch(e2){await cart.coupon.set(payload)}applied=true}else if(cart&&typeof cart.setCoupon===\"function\"){try{await cart.setCoupon(c)}catch(e3){await cart.setCoupon(payload)}applied=true}else if(cart&&typeof cart.addCoupon===\"function\"){try{await cart.addCoupon(c)}catch(e4){await cart.addCoupon(payload)}applied=true}else if(window.salla&&typeof window.salla.applyCoupon===\"function\"){try{await window.salla.applyCoupon(c)}catch(e5){await window.salla.applyCoupon(payload)}applied=true}else if(window.salla&&window.salla.coupon&&typeof window.salla.coupon.apply===\"function\"){try{await window.salla.coupon.apply(c)}catch(e6){await window.salla.coupon.apply(payload)}applied=true}if(applied)return true}catch(e){warn(\"bundle-app: coupon apply failed\",e&&((e.details)||e.message||e))}await sleep(450+attempt*250)}return false}"
    );
    parts.push(
      "async function tryClearCoupon(){try{var cart=window.salla&&window.salla.cart;if(cart&&cart.coupon&&typeof cart.coupon.remove===\"function\"){await cart.coupon.remove();return true}if(cart&&typeof cart.removeCoupon===\"function\"){await cart.removeCoupon();return true}if(cart&&typeof cart.clearCoupon===\"function\"){await cart.clearCoupon();return true}if(cart&&cart.coupon&&typeof cart.coupon.set===\"function\"){try{await cart.coupon.set(\"\")}catch(e0){await cart.coupon.set({code:\"\"})}return true}if(cart&&typeof cart.applyCoupon===\"function\"){try{await cart.applyCoupon(\"\");return true}catch(e){try{await cart.applyCoupon({code:\"\"});return true}catch(e2){}}}}catch(e){warn(\"bundle-app: clear coupon failed\",e&&((e.details)||e.message||e))}return false}"
    );
    parts.push(
      "async function addItemsToCart(items){var cart=window.salla&&window.salla.cart;if(!cart||typeof cart.addItem!==\"function\")throw new Error(\"Salla cart API not available\");for(var i=0;i<items.length;i++){var it=items[i]||{};var qty=Math.max(1,Math.floor(Number(it.quantity||1)));var pidNum=Number(it.productId);try{if(it.productId&&Number.isFinite(pidNum)&&pidNum>0){await cart.addItem({id:pidNum,quantity:qty});continue}await cart.addItem({id:String(it.variantId),quantity:qty})}catch(e){try{await cart.addItem({id:String(it.variantId),quantity:qty})}catch(e2){throw e2}}}}"
    );
    ...
    parts.push(
      "function bundleKind(bundle){try{var raw=String(bundle&&bundle.kind||\"\").trim();if(raw)return raw;var offer=bundle&&bundle.offer&&typeof bundle.offer===\"object\"?bundle.offer:{};if(Array.isArray(offer.tiers)&&offer.tiers.length)return\"quantity_discount\";var v=Number((offer&&offer.value!=null)?offer.value:0);if(Number.isFinite(v)&&v<=0)return\"products_no_discount\";return\"products_discount\"}catch(e){return\"products_discount\"}}"
    );
    parts.push(
      "function computeBundleApplyItems(bundle){try{if(!bundle)return null;var kind=bundleKind(bundle);var items=normalizeItems(bundle);items=Array.isArray(items)?items:[];if(kind===\"post_add_upsell\"){var out=[];for(var i=0;i<items.length;i++){var it=items[i]||{};if(it&&it.isBase===true)continue;out.push(it)}return out}return items}catch(e){return null}}"
    );
    parts.push(
    ...
    parts.push(
      "async function applyBundleSelection(bundle){var bid=String(bundle&&bundle.id||\"\");var trigger=String(bundle&&bundle.triggerProductId||\"\");var kind=String(bundleKind(bundle)||\"\").trim();if(!bid)return;if(!trigger&&(kind!==\"products_no_discount\"&&kind!==\"post_add_upsell\"))return;selectedBundleId=bid;applying=true;try{messageByBundleId[bid]='جاري إضافة الباقة...';renderProductBanners(lastBundles||[])}catch(e){}try{var items=(typeof computeBundleApplyItems===\"function\"?computeBundleApplyItems(bundle):null)||normalizeItems(bundle);items=Array.isArray(items)?items:[];if(!items.length){messageByBundleId[bid]='لا يوجد عناصر لإضافتها للسلة';applying=false;try{renderProductBanners(lastBundles||[])}catch(e0){}return}if(kind===\"products_no_discount\"||kind===\"post_add_upsell\"){try{await addItemsToCart(items);messageByBundleId[bid]='تمت إضافة المنتجات للسلة'}catch(addErr){messageByBundleId[bid]='لم يتم الإضافة'}applying=false;try{renderProductBanners(lastBundles||[])}catch(e01){}return}var prev=loadSelection(trigger);await tryClearCoupon();if(prev&&prev.bundleId&&String(prev.bundleId)!==bid&&Array.isArray(prev.items)&&prev.items.length){await removeItemsFromCart(prev.items)}await addItemsToCart(items);messageByBundleId[bid]='جاري تجهيز الخصم...';try{renderProductBanners(lastBundles||[])}catch(e2){}var res=await requestApplyBundle(bid,items.map(function(it){return{variantId:it.variantId,quantity:it.quantity}}));var hasDiscount=Boolean(res&&res.hasDiscount&&res.couponCode);var issueFailed=Boolean(res&&res.couponIssueFailed);if(hasDiscount){saveSelection(trigger,{bundleId:bid,triggerProductId:trigger,items:items,ts:Date.now()});savePendingCoupon(trigger,{code:String(res.couponCode),ts:Date.now()});var ok=await tryApplyCoupon(String(res.couponCode));if(ok){clearPendingCoupon(trigger);messageByBundleId[bid]='تم تطبيق الخصم على السلة'}else{messageByBundleId[bid]='تم تجهيز الكوبون، افتح السلة وسيتم تطبيقه تلقائيًا'}}else if(issueFailed){messageByBundleId[bid]='تمت إضافة الباقة لكن تعذر إنشاء كوبون الخصم. حاول مرة أخرى.';clearPendingCoupon(trigger)}else{messageByBundleId[bid]='تمت إضافة الباقة. لا يوجد خصم لهذه الباقة.';clearPendingCoupon(trigger)}selectedBundleId=bid}catch(e){warn(\"bundle-app: apply bundle failed\",e&&((e.details)||e.message||e));messageByBundleId[bid]='حصل خطأ أثناء إضافة الباقة أو تطبيق الخصم'}applying=false;try{renderProductBanners(lastBundles||[])}catch(e){} }"
    );
...
function createApiRouter(config) {
  ...
  function buildStorefrontSnippetJsV2(merchantId, token) {
    ...
    parts.push(
      "async function tryApplyCoupon(code){var c=String(code||\"\").trim();if(!c)return false;function sleep(ms){return new Promise(function(r){setTimeout(r,ms)})}for(var attempt=0;attempt<6;attempt++){try{var cart=window.salla&&window.salla.cart;var applied=false;var payload={code:c};if(cart&&typeof cart.applyCoupon===\"function\"){try{await cart.applyCoupon(c)}catch(e0){await cart.applyCoupon(payload)}applied=true}else if(cart&&cart.coupon&&typeof cart.coupon.apply===\"function\"){try{await cart.coupon.apply(c)}catch(e1){await cart.coupon.apply(payload)}applied=true}else if(cart&&cart.coupon&&typeof cart.coupon.set===\"function\"){try{await cart.coupon.set(c)}catch(e2){await cart.coupon.set(payload)}applied=true}else if(cart&&typeof cart.setCoupon===\"function\"){try{await cart.setCoupon(c)}catch(e3){await cart.setCoupon(payload)}applied=true}else if(cart&&typeof cart.addCoupon===\"function\"){try{await cart.addCoupon(c)}catch(e4){await cart.addCoupon(payload)}applied=true}else if(window.salla&&typeof window.salla.applyCoupon===\"function\"){try{await window.salla.applyCoupon(c)}catch(e5){await window.salla.applyCoupon(payload)}applied=true}else if(window.salla&&window.salla.coupon&&typeof window.salla.coupon.apply===\"function\"){try{await window.salla.coupon.apply(c)}catch(e6){await window.salla.coupon.apply(payload)}applied=true}if(applied)return true}catch(e){var st=null;var msg=\"\";try{st=Number((e&&((e.statusCode!=null&&e.statusCode)||(e.status!=null&&e.status)||(e.response&&e.response.status)))||null)}catch(x){}try{msg=String((e&&((e.details&&e.details.message)||e.message))||\"\")}catch(x2){}msg=String(msg||\"\");if(st===410||msg.indexOf('لم يعد متاحا')!==-1||msg.toLowerCase().indexOf('no longer available')!==-1){return false}warn(\"bundle-app: coupon apply failed\",e&&((e.details)||e.message||e))}await sleep(400+attempt*250)}return false}"
    );
    parts.push(
      "function pendingCouponKey(){return \"bundle_app_pending_coupon:\"+String(merchantId||\"\")}"
    );
  ...
  function buildStorefrontSnippetJsManualBundles(merchantId, token) {
    ...
    parts.push(
      "async function tryApplyCoupon(code){var c=String(code||\"\").trim();if(!c)return false;if(storeClosedNow())return false;function sleep(ms){return new Promise(function(r){setTimeout(function(){r()},ms)})}for(var attempt=0;attempt<3;attempt++){try{var cart=window.salla&&window.salla.cart;var applied=false;var payload={code:c};if(cart&&typeof cart.applyCoupon===\"function\"){try{await cart.applyCoupon(c)}catch(e0){await cart.applyCoupon(payload)}applied=true}else if(cart&&cart.coupon&&typeof cart.coupon.apply===\"function\"){try{await cart.coupon.apply(c)}catch(e1){await cart.coupon.apply(payload)}applied=true}else if(cart&&cart.coupon&&typeof cart.coupon.set===\"function\"){try{await cart.coupon.set(c)}catch(e2){await cart.coupon.set(payload)}applied=true}else if(cart&&typeof cart.setCoupon===\"function\"){try{await cart.setCoupon(c)}catch(e3){await cart.setCoupon(payload)}applied=true}else if(cart&&typeof cart.addCoupon===\"function\"){try{await cart.addCoupon(c)}catch(e4){await cart.addCoupon(payload)}applied=true}else if(window.salla&&typeof window.salla.applyCoupon===\"function\"){try{await window.salla.applyCoupon(c)}catch(e5){await window.salla.applyCoupon(payload)}applied=true}else if(window.salla&&window.salla.coupon&&typeof window.salla.coupon.apply===\"function\"){try{await window.salla.coupon.apply(c)}catch(e6){await window.salla.coupon.apply(payload)}applied=true}if(applied){try{g.BundleApp._lastCouponApplyStatus=null;g.BundleApp._lastCouponApplyMessage=\"\"}catch(x0){}return true}}catch(e){var st=extractHttpStatus(e);var msg=extractHttpMessage(e);try{g.BundleApp._lastCouponApplyStatus=st;g.BundleApp._lastCouponApplyMessage=String(msg||\"\")}catch(x1){}markStoreClosed({status:st,message:msg});if(storeClosedNow())return false;warn(\"bundle-app: coupon apply failed\",e&&((e.details)||e.message||e))}await sleep(250+attempt*250)}return false}"
    );
    parts.push(
      "async function tryClearCoupon(){if(storeClosedNow())return false;try{var cart=window.salla&&window.salla.cart;if(cart&&cart.coupon&&typeof cart.coupon.remove===\"function\"){await cart.coupon.remove();return true}if(cart&&typeof cart.removeCoupon===\"function\"){await cart.removeCoupon();return true}if(cart&&typeof cart.clearCoupon===\"function\"){await cart.clearCoupon();return true}if(cart&&cart.coupon&&typeof cart.coupon.set===\"function\"){try{await cart.coupon.set(\"\")}catch(e0){await cart.coupon.set({code:\"\"})}return true}if(cart&&typeof cart.applyCoupon===\"function\"){try{await cart.applyCoupon(\"\");return true}catch(e){try{await cart.applyCoupon({code:\"\"});return true}catch(e2){markStoreClosed(e);if(storeClosedNow())return false}}}}catch(e){markStoreClosed(e);warn(\"bundle-app: clear coupon failed\",e&&((e.details)||e.message||e))}return false}"
    );
    parts.push(
    ...
    parts.push(
      "function markStoreClosed(e){try{var msg=extractHttpMessage(e);var m=String(msg||\"\");var ml=m.toLowerCase();if(m.indexOf('المتجر مغلق')!==-1||ml.indexOf('store is closed')!==-1){g.BundleApp._storeClosedUntil=Date.now()+60000}}catch(x){}}"
    );
    parts.push(
